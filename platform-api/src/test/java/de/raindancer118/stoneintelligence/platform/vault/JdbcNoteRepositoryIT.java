...
class JdbcNoteRepositoryIT extends NoteRepositoryContractTest {
    ...
    static void migrateAndBuildRepository() {
        ...
        repository = new JdbcNoteRepository(JdbcClient.create(dataSource));
    }

    @BeforeEach
    void cleanTables() {
        JdbcClient.create(dataSourceOf()).sql("TRUNCATE platform.note_tombstones, platform.notes CASCADE").update();
    }

    private DataSource dataSourceOf() {
