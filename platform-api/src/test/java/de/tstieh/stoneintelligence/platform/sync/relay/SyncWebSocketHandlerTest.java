...
class SyncWebSocketHandlerTest {
    ...
    void should_neitherJoinNorAcceptUpdatesForAFile() {
        ...
        var file = notes.create(vaultId, "Skript.pdf", NoteLevel.of(1), "creator",
            de.tstieh.stoneintelligence.platform.vault.NoteKind.FILE);
        var role = authorization.createRole(vaultId, "writer", Set.of(Permission.READ, Permission.WRITE));
        var group = authorization.createGroup(vaultId, "writers");
        authorization.assignRole(group.id(), role.id());
        authorization.addMember(group.id(), "writer-actor");
    ...
    class AccessChanges {
        ...
        private static final de.tstieh.stoneintelligence.platform.identity.GrantScope READER =
            de.tstieh.stoneintelligence.platform.identity.GrantScope.user("reader-actor");

        private de.tstieh.stoneintelligence.platform.identity.GrantTarget target(NoteId noteId) {
            return de.tstieh.stoneintelligence.platform.identity.GrantTarget.entry(noteId, "shared.md");
        }
