...
class VaultControllerTest {
    ...
    private final de.tstieh.stoneintelligence.platform.invitation.RecordingMailer mailer =
        new de.tstieh.stoneintelligence.platform.invitation.RecordingMailer();
    private final de.tstieh.stoneintelligence.platform.invitation.InvitationService invitations =
        new de.tstieh.stoneintelligence.platform.invitation.InvitationService(
            new de.tstieh.stoneintelligence.platform.invitation.FakeInvitationRepository(),
            new de.tstieh.stoneintelligence.platform.invitation.FakeUserDirectory(), mailer, authorization,
            new VaultAccessGuard(authorization, new de.tstieh.stoneintelligence.platform.identity.FakeAccessGrantRepository(new de.tstieh.stoneintelligence.platform.vault.FakeNoteRepository())), vaults, java.time.Clock.systemUTC(),
            new de.tstieh.stoneintelligence.platform.invitation.InvitationSettings("https://kb.example", java.time.Duration.ofDays(14)));
    private final VaultController controller = new VaultController(vaults, authorization, invitations,
        new VaultAccessGuard(authorization, new de.tstieh.stoneintelligence.platform.identity.FakeAccessGrantRepository(new de.tstieh.stoneintelligence.platform.vault.FakeNoteRepository())));
    ...
    void should_grantCreatorFullPermissions_when_vaultIsCreated() {
        ...
        var vaultId = de.tstieh.stoneintelligence.domain.id.VaultId.of(response.id());
    ...
    void should_acceptPendingInvitationsForTheSignedInEmail_whenListingVaults() {
        ...
        invitations.inviteByEmail(de.tstieh.stoneintelligence.domain.id.VaultId.of(vault.id()), "tom", "neu@example.org",
            de.tstieh.stoneintelligence.platform.invitation.InviteAccess.EDIT);
    ...
    void should_renameAVault_onlyForWhoManagesIt() {
        ...
        var group = authorization.createGroup(de.tstieh.stoneintelligence.domain.id.VaultId.of(created.id()), "guests");
        ...
        org.assertj.core.api.Assertions.assertThatThrownBy(() -> controller.rename(created.id(), new VaultController.RenameVaultRequest(" "), tom))
            .isInstanceOf(de.tstieh.stoneintelligence.platform.identity.InvalidGrantException.class);
    }
}
