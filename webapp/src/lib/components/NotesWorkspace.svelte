...
<div class="workspace-tools" class:note-open={selected !== null}>
  <div><h2 class="section-title">Deine Notizen</h2><p class="hint">{loading && !notes.length ? "Übersicht wird geladen…" : `${notes.length} ${notes.length === 1 ? "Notiz" : "Notizen"} geladen${complete ? "" : " · weitere verfügbar"}`}</p></div>
...
<div class="notes-layout" class:has-selection={selected !== null}>
  <section class="note-index" aria-label="Notizliste">
    ...
    <div class="note-rows" aria-busy={loading}>
      {#each visible as note (note.id)}<button class="note-row" class:active={selected?.id === note.id} aria-current={selected?.id === note.id ? "true" : undefined} onclick={() => select(note)}><span class="note-name">{note.path.split("/").pop()?.replace(/\.md$/i, "")}</span><span class="note-path">{note.path.includes("/") ? note.path.slice(0, note.path.lastIndexOf("/")) : "Ohne Ordner"}</span>{#if note.noteLevel === 101}<span class="locked">Verschlüsselt</span>{/if}</button>{/each}
  ...
  <section class="note-content" aria-label="Ausgewählte Notiz">
    {#if selected}<button class="quiet back-to-list" onclick={() => select(null)}>Zur Notizliste</button>{#key selected.id}<NoteEditor note={selected} {permissions} onChanged={changed} onDeleted={deleted} onDirtyChange={setDirty} />{/key}
...
<style>
  ...
  .note-name { font-weight: 500; overflow-wrap: anywhere; } .note-path, .locked { font-size: .75rem; color: var(--ink-dim); overflow-wrap: anywhere; } .locked { color: var(--rust); }
