...
public final class VaultWriter {
    ...
    private String renderBlock(DraftNote note, Path self, String sourceLink, Path original) {
        ...
        if (note.definition() != null && !note.definition().isBlank()) {
            block.append("> ").append(resolveLinks(de.tstieh.stoneintelligence.stoneai.note.MathDelimiters.forObsidian(note.definition().strip()))
                    .replace("\n", "\n> ")).append("\n\n");
        }
        block.append(escapeLinksInTables(resolveLinks(de.tstieh.stoneintelligence.stoneai.note.MathDelimiters.forObsidian(note.body().strip()))));
